package Stormpath::Client::UserAgentProxy;
use strict;
use Carp;
use HTTP::Request;
...
sub request {
    ...
    if($self->method() eq 'basic') {

        $http_request->authorization_basic($self->id(), $self->secret());

        return $ua->request($http_request);

    } elsif($self->method() eq 'SAuthc1') {

        return $ua->request($self->sign_request($http_request)); 

    } else {

        Carp::confess("unknown method");
